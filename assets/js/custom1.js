...
(function ($) {
	...
	$('#myonoffswitch55').on('click', function () {
		if (this.checked) {
			...
			(document.getElementById("style")?.setAttribute("href", "http://127.0.0.1:8000/static/assets//plugins/bootstrap/css/bootstrap.rtl.css"));
	...
	$('#myonoffswitch54').on('click', function () {
		if (this.checked) {
			...
			$("head link#style").attr("href", $(this));
			(document.getElementById("style")?.setAttribute("href", "http://127.0.0.1:8000/static/assets//plugins/bootstrap/css/bootstrap.css"));
			var carousel = $('.owl-carousel');
			if (carousel) {
				$.each(carousel, function (index, element) {
					// element == this
...
(()=>{
	window.addEventListener('DOMContentLoaded', ()=>{
		if(document.querySelector('#global-loader')){
			document.querySelector('#global-loader').classList.add('d-none');
		}
	})
})()
